  ...
  @table_names = {}
    ...
    for cls in models do
        ...
        for association in associations do
          ...
          if a["belongs_to => "+name+"->"+association.name.to_s] == nil
            print "adding "+association.macro.to_s+" --->"+name+"->"+association.name.to_s+"\n"     
          	...
          	str = "    if @params['internal'] == '"+association.name.to_s.pluralize+"'\n"
						str +="      render :partial => \""+association.name.to_s.pluralize+"/edit_contents\", :locals => {'@"+association.name.to_s.singularize+"' => "+association.name.to_s.singularize.camelize+".find(@params['pk'])}\n"
						str +="    end\n"
          	...
          	@table_names[name]["addrelated"][association.name] = str
						...
						#str += "     query = \"\""
						str =  "     @"+association.name.to_s.pluralize+" = nil\n"
            ...
            str += "       if query == [\"\"]\n"
       			str += "         @"+association.name.to_s.pluralize+" = "+association.name.to_s.singularize.camelize+".find_all\n"
     				str += "       else\n"
     				str += "         @"+association.name.to_s.pluralize+" = "+association.name.to_s.singularize.camelize+".find :all, :conditions => query\n"
     				str += "       end\n"
     				str += "     end\n"
            ...
            case association.macro
              when :belongs_to then
                ...
                target_port = "head"
								...
								str += "      @"+name+".save\n"
								str += "      render_component :controller => \""+association.name.to_s.pluralize+"\", :action => \"edit_dynamic\", :id => o.id, :params => {'internal' => \""+name.pluralize+"\", 'pk' => @params['id'], 'relatedtype'=> '"+association.name.to_s.singularize+"', 'level' => @params['level'], 'new' => 'yes'}\n"
								str += "    end"
     ...
     @table_names.each do |name, name_value|
      ...
      if name_value["deleterelatedaction"] != nil
    		name_value["deleterelatedaction"].each do |title, value|
    			file.puts "  helper :"+title.to_s.pluralize+"\n"
    		end
    	end

	    file.puts "\n\n  "+name.to_s.singularize.camelcase+".content_columns.each do |column|\n"
	    file.puts "    in_place_update_form :"+name.to_s.singularize+", column.name\n"
	    file.puts "  end\n"
      if name_value["deleterelatedaction"] != nil
  			name_value["deleterelatedaction"].each do |title, value|
  		    file.puts "  "+title.to_s.singularize.camelcase+".content_columns.each do |column|\n"
  		    file.puts "    in_place_update_form :"+title.to_s.singularize+", column.name\n"
  		    file.puts "  end\n"
  			end
  		end
   		...
   		file.puts "    else\n"
			...
			file.puts "    @"+name+".created_by = @session[:user].login\n"
			file.puts "    @"+name+".created_at = Time.now\n"
			file.puts "    @"+name+".save\n"
			file.puts "    redirect_to :action => 'edit_dynamic', :id => @"+name+".id, :params => {'new' => 'true'}\n"
			file.puts "  end\n\n"
      ...
      file.puts "      @params['level'] = (@params['level'].to_i + 1).to_s\n"
    	...
    	file.puts "    end\n"
			...
			file.puts "    @"+name+" = "+name.camelize+".find(params[:id])\n"
			file.puts "    if @"+name+".created_by == nil or @"+name+".created_by == \"\"\n"
			file.puts "      @"+name+".created_by = @session[:user].login\n"
			file.puts "    @"+name+".created_at = Time.now\n"
			file.puts "    end\n"
			file.puts "    if @session[:user] != nil\n"
			file.puts "      @"+name+".updated_by = @session[:user].login\n"
			file.puts "    end\n"
			file.puts "    @"+name+".updated_at = Time.now\n"
			file.puts "    if @"+name+".update_history == nil\n"
			file.puts "      @"+name+".update_history = @session[:user].login + \" [\"+Time.now.to_s+\"]\n\"\n"
			file.puts "    else\n"
			file.puts "    	@"+name+".update_history += @session[:user].login + \" [\"+Time.now.to_s+\"]\n\"\n"
			file.puts "    end\n"
      ...
      if name_value["deleterelatedaction"] != nil
          ...
          file.puts "      if flash['"+title+"'] != nil\n"
          file.puts "        @params['"+title+"'] = flash['"+title+"']\n"
          file.puts "        flash['relatedsearch'] = 'yes'\n"
          file.puts "      end\n"
        end
      end
      ...
      if name_value["deleterelatedaction"] != nil
			...
			file.puts "<% else %>\n"
			#file.puts "<%= link_to_function (image_tag(\"update_up.gif\", :size => \"87x19\", :border => 0, :alt => \"update\", :name => \""+name+"update1\"+@params['level'].to_s), \"#\", {:onMouseOut => \"MM_swapImgRestore();\", :onMouseOver => \"MM_swapImage('"+name+"update1\"+@params['level'].to_s+\"','','/images/update_down.gif',1);\"}) %>"
      ...
      if name_value["deleterelatedaction"] != nil
          ...
          file.puts "  <% end %>\n"
  				...
  				if @hasmany[name+"->"+key.to_s] != nil
  					...
  					file.puts "<div id=\"scroll"+key.to_s+"\" style=\"height:100;background-color:e6e6e6;border:1px solid;overflow:auto;\">\n"
  					file.puts "<% end %>\n"
			...
			file.puts "</div>"
			file.puts "<% if @params['internal'] == nil %>\n"
			file.puts "<p><b>Created by:</b> <%= @"+name+".created_by %> [<%= @"+name+".created_at %>]<br>\n"
			file.puts "<b>Last updated by:</b> <%= @"+name+".updated_by %> [<%= @"+name+".updated_at %>]<br>\n"
			file.puts "<span id=\"show_history\">\n"
			file.puts "<a href=\"#\" onClick=\"Element.hide('show_history');Effect.BlindDown('history');\"><b>Show History</b></a><br>\n"
			file.puts "</span>\n"
			file.puts "<div id=\"history\" style=\"display:none;\">\n"
			file.puts "<div>\n"
			file.puts "<b>Update History:</b><a href=\"#\" onClick=\"Effect.BlindUp('history');Element.show('show_history')\">(Hide)</a><br>\n"
			file.puts "<div style=\"width:400;height:70;background-color:eeeeee;border:1px solid;overflow:auto;\">\n"
			file.puts "<%= textilize @"+name+".update_history %>\n"
			file.puts "</div>\n"
			file.puts "</div>\n"
			file.puts "</div></p>\n"
			file.puts "<% end %>\n"
			...
			# write edit file
			file = File.new("temp/"+name+"/edit_dynamic.rhtml", "w") 
			file.puts "<SCRIPT language=\"javascript\">document.header.mode=\"edit\";</SCRIPT>\n"
			#file.puts "<% if @params['internal'] != nil %>\n"
			#file.puts "<div class=\"internal\">\n"
			#file.puts "<% end %>\n"
			...
			file.puts "<%= error_messages_for '"+name+"' %>\n"
			file.puts "<% if @params['internal'] == nil or @params['internal'] == ''%>\n"
			file.puts "  <%= form_tag({:controller => \""+name.pluralize+"\", :action => \"update\"}, {:name => \"edit"+name+"\", :onsubmit => \"return check_windows('"+name.singularize+"')\"})  %>\n"
			file.puts "<% else %>\n"
			file.puts "  <%= form_remote_tag(:update => @divname, :complete => \"new Effect.Fade('\"+@divname+\"search_"+name+"');new Effect.Fade('\"+@divname+\"update_"+name+"');\", :url => {:controller => '"+name.pluralize+"', :action => 'update', :id => @params[:id], :params => {:internal => @params['internal'], :pk => @params['pk'], 'relatedtype'=> @params['relatedtype'], 'level' => @params['level']}}, :condition => \"open_windows['"+name.singularize+"'].length > 0 ? alert('You have unsaved changes.') : true\")  %>\n"
			file.puts "<% end %>\n"
			file.puts "<%= image_submit_tag \"update_up.gif\", :size => \"87x19\", :border => 0 %>\n"
			#file.puts "<%= link_to_function(image_tag(\"update_up.gif\", :size => \"87x19\", :border => 0, :alt => \"update\", :name => \""+name+"update1\"+@params['level'].to_s), \"document.forms[\"+@params['level'].to_s+\"].submit()\", {:onMouseOut => \"MM_swapImgRestore();\", :onMouseOver => \"MM_swapImage('"+name+"update1\"+@params['level'].to_s+\"','','/images/update_down.gif',1);\"}) %>"
			file.puts "<% if @params['internal'] != nil %>\n"
			file.puts "  <%= link_to_remote image_tag(\"cancel_up.gif\", :size => \"87x19\", :border => 0, :alt => \"cancel\", :name => \""+name+"cancel0\"+@params['level'].to_s), {:update => @divname, :complete => \"new Effect.Fade('\"+@divname+\"search_"+name+"');new Effect.Fade('\"+@divname+\"update_"+name+"');\", :url => {:controller => '"+name.pluralize+"', :action => 'cancel', :id => @params[:id], :params => {:internal => @params['internal'], :pk => @params['pk'], 'relatedtype'=> @params['relatedtype'], 'level' => @params['level'], 'new' => @params['new']}}}, :onMouseOut => \"MM_swapImgRestore();\", :onMouseOver => \"MM_swapImage('"+name+"cancel0\"+@params['level'].to_s+\"','','/images/cancel_down.gif',1);\"%>"
			file.puts "<% else %>\n"
			file.puts "  <%= link_to image_tag(\"cancel_up.gif\", :size => \"87x19\", :border => 0, :alt => \"Cancel\", :name => \"cancelz\"), {:controller => '"+name.pluralize+"', :action => 'cancel', :id => @params[:id], :params => {'new' => @params['new']}}, :onMouseOut => \"MM_swapImgRestore();\", :onMouseOver => \"MM_swapImage('cancelz','','/images/cancel_down.gif',1);\", :confirm => \"Abandon any changes?\"%>\n"
			file.puts "<% end %>\n"
      ...
      if name_value["deleterelatedaction"] != nil
        ...
        file.puts "  <% end %>\n"
				...
				if @hasmany[name+"->"+key.to_s] != nil
					file.puts "<% if @"+name+"."+key.to_s+".size > 5 %>\n"
					file.puts "<div id=\"scroll"+key.to_s+"\" style=\"height:70;background-color:e6e6e6;border:0px solid;overflow:auto;\">\n"
					file.puts "<% end %>\n"
					file.puts "<% for o in @"+name+"."+key.to_s+" %>\n"
        file.puts "    <%= link_to_remote o.displayInfo, :update => \""+name+"_"+key.to_s.pluralize+"\", :url => {:controller => \""+key.to_s.pluralize+"\", :action => \"edit_dynamic\", :id => o.id , :params => {:level => @params['level'], :relatedtype => \""+key.to_s.singularize+"\", :internal => '"+name.to_s.pluralize+"', 'pk' => @params['id']}} %><br>\n"
					#file.puts "  <%= link_to o.displayInfo, :controller=> \""+key.to_s+"\", :action => \"show_edit\", :id => o.id %><br>\n"
					file.puts "<% end %>\n"
					file.puts "<% if @"+name+"."+key.to_s+".size > 5 %>\n"
					file.puts "</div>\n"
					file.puts "<% end %>\n"
					...
					file.puts "<% if !@"+name+"."+key.to_s+".nil? %>\n"
          file.puts "    <%= link_to_remote @"+name+"."+key.to_s+".displayInfo, :update => \""+name+"_"+key.to_s.pluralize+"\", :url => {:controller => \""+key.to_s.pluralize+"\", :action => \"edit_dynamic\", :id => @"+name+"."+key.to_s+".id , :params => {:level => @params['level'], :relatedtype => \""+key.to_s.singularize+"\", :internal => '"+name.to_s.pluralize+"','pk' => @params['id']}} %>\n"
					#file.puts "  <%= link_to @"+name+"."+key.to_s+".displayInfo, :controller => \""+key.to_s.pluralize+"\", :action => \"show_edit\",  :id => @"+name+"."+key.to_s+".id  %>\n"
					file.puts "<% end %>\n"
				end
				file.puts "</div>\n"
				file.puts "<% end %>\n"
			...
			# write show_edit file
			file = File.new("temp/"+name+"/show_edit.rhtml", "w") 
			file.puts "<SCRIPT language=\"javascript\">document.header.mode=\"edit\";</SCRIPT>\n"
			#file.puts "<% if @params['internal'] != nil %>\n"
			#file.puts "<div class=\"internal\">\n"
			#file.puts "<% end %>\n"
			...
			file.puts "<% if @params['internal'] != nil %>\n"
			file.puts "  <%= link_to_remote image_tag(\"update_up.gif\", :size => \"87x19\", :border => 0, :alt => \"cancel\", :name => \""+name+"update0\"+@params['level'].to_s), {:update => @divname, :complete => \"new Effect.Fade('\"+@divname+\"search_"+name+"');new Effect.Fade('\"+@divname+\"update_"+name+"');\", :url => {:controller => '"+name.pluralize+"', :action => 'update_related', :id => @params[:id], :params => {:internal => @params['internal'], :pk => @params['pk'], 'relatedtype'=> @params['relatedtype'], 'level' => @params['level']}}}, :onMouseOut => \"MM_swapImgRestore();\", :onMouseOver => \"MM_swapImage('"+name+"update0\"+@params['level'].to_s+\"','','/images/update_down.gif',1);\"%>"
			file.puts "  <%= link_to_remote image_tag(\"cancel_up.gif\", :size => \"87x19\", :border => 0, :alt => \"cancel\", :name => \""+name+"cancel0\"+@params['level'].to_s), {:update => @divname, :complete => \"new Effect.Fade('\"+@divname+\"search_"+name+"');new Effect.Fade('\"+@divname+\"update_"+name+"');\", :url => {:controller => '"+name.pluralize+"', :action => 'cancel', :id => @params[:id], :params => {:internal => @params['internal'], :pk => @params['pk'], 'relatedtype'=> @params['relatedtype'], 'level' => @params['level']}}}, :onMouseOut => \"MM_swapImgRestore();\", :onMouseOver => \"MM_swapImage('"+name+"cancel0\"+@params['level'].to_s+\"','','/images/cancel_down.gif',1);\"%>"
			file.puts "<% else %>\n"
			file.puts "<%= link_to image_tag(\"back_up.gif\", :size => \"87x19\", :border => 0, :alt => \"back\", :name => \"back1\"), @session['return_to'] , :onMouseOut => \"MM_swapImgRestore();\", :onMouseOver => \"MM_swapImage('back1','','/images/back_down.gif',1);\" %>\n"
			file.puts "<% end %>\n"
			file.puts "<h1>Edit "+name.humanize+"</h1>\n"
			file.puts "<%= error_messages_for '"+name+"' %>\n"
			...
			#file.puts "<div id=\"scrollwindow<%= @params['level'] %>\" style=\"height:400;padding:5px;border:1px solid;overflow:auto;\">\n"
			file.puts "<%= show_edit_"+name+" %>\n"
			#file.puts "</div>\n"

			file.puts "<% if @params['internal'] != nil %>\n"
			file.puts "<br><br>  <%= link_to_remote image_tag(\"update_up.gif\", :size => \"87x19\", :border => 0, :alt => \"cancel\", :name => \""+name+"update1\"+@params['level'].to_s), {:update => @divname, :complete => \"new Effect.Fade('\"+@divname+\"search_"+name+"');new Effect.Fade('\"+@divname+\"update_"+name+"');\", :url => {:controller => '"+name.pluralize+"', :action => 'update_related', :id => @params[:id], :params => {:internal => @params['internal'], :pk => @params['pk'], 'relatedtype'=> @params['relatedtype'], 'level' => @params['level']}}}, :onMouseOut => \"MM_swapImgRestore();\", :onMouseOver => \"MM_swapImage('"+name+"update1\"+@params['level'].to_s+\"','','/images/update_down.gif',1);\"%>"
			file.puts "<%= link_to_remote image_tag(\"cancel_up.gif\", :size => \"87x19\", :border => 0, :alt => \"cancel\", :name => \""+name+"cancel2\"+@params['level'].to_s), {:update => @divname, :complete => \"new Effect.Fade('\"+@divname+\"search_"+name+"');new Effect.Fade('\"+@divname+\"update_"+name+"');\", :url => {:controller => '"+name.pluralize+"', :action => 'cancel', :id => @params[:id], :params => {:internal => @params['internal'], :pk => @params['pk'], 'relatedtype'=> @params['relatedtype'], 'level' => @params['level']}}}, :onMouseOut => \"MM_swapImgRestore();\", :onMouseOver => \"MM_swapImage('"+name+"cancel2\"+@params['level'].to_s+\"','','/images/cancel_down.gif',1);\"%>"
			file.puts "<% else %>\n"
			file.puts "<br><br><%= link_to image_tag(\"back_up.gif\", :size => \"87x19\", :border => 0, :alt => \"back\", :name => \"back2\"), @session['return_to'] , :onMouseOut => \"MM_swapImgRestore();\", :onMouseOver => \"MM_swapImage('back2','','/images/back_down.gif',1);\" %>\n"
      file.puts "<% end %>\n"
			str = "@params[\"query\"] != nil"
			name_value['addrelated'].each_key do |key|
				str += " or @params['"+key.to_s+"'] != nil"
        ...
        file.puts "  <% end %>\n"
				...
				if @hasmany[name+"->"+key.to_s] != nil
					...
					file.puts "<% end %>\n"
					file.puts "<% for o in @"+name+"."+key.to_s+" %>\n"
        file.puts "    <%= link_to_remote o.displayInfo, :update => \""+name+"_"+key.to_s.pluralize+"\", :url => {:controller => \""+key.to_s.pluralize+"\", :action => \"show_edit\", :id => @"+name+"."+key.to_s+".id , :params => {:level => @params['level'], :relatedtype => \""+key.to_s.singularize+"\", :internal => 'internal'}} %>\n"
					#file.puts "  <%= link_to o.displayInfo, :controller=> \""+key.to_s+"\", :action => \"show_edit\", :id => o.id %><br>\n"
					file.puts "<% end %>\n"
					file.puts "<% if @"+name+"."+key.to_s+".size > 5 %>\n"
					file.puts "</div>\n"
					...
					file.puts "<% if !@"+name+"."+key.to_s+".nil? %>\n"
        file.puts "    <%= link_to_remote @"+name+"."+key.to_s+".displayInfo, :update => \""+name+"_"+key.to_s.pluralize+"\", :url => {:controller => \""+key.to_s.pluralize+"\", :action => \"show_edit\", :id => @"+name+"."+key.to_s+".id , :params => {:level => @params['level'], :relatedtype => \""+key.to_s.singularize+"\", :internal => 'internal'}} %>\n"
					#file.puts "  <%= link_to @"+name+"."+key.to_s+".displayInfo, :controller => \""+key.to_s.pluralize+"\", :action => \"show_edit\",  :id => @"+name+"."+key.to_s+".id  %>\n"
					file.puts "<% end %>\n"
				end     
				file.puts "</div>"
			...
			file.puts "<%= form_tag({:action => \"searchaction\"}, {:method => 'get',:name => \"searchform\"})  %>\n"   
			...
			#write list file
			file = File.new("temp/"+name+"/list.rhtml", "w") 

			file.puts "  <h1>Listing "+name.humanize+"</h1>\n"
			file.puts "  <%= render_partial \"partial/paginator\", @"+name+"_pages %><br>\n"
			file.puts "<%= list_"+name+" %>\n"

			file.puts "  <br /><%= render_partial \"partial/paginator\", @"+name+"_pages %><br />\n"
      file.close
			...
			file.puts "<%= link_to image_tag(\"edit_up.gif\", :size => \"87x19\", :border => 0, :alt => \"edit\", :name => \"edit1\"), {:action => 'edit_dynamic', :id => @"+name+".id}, :onMouseOut => \"MM_swapImgRestore();\", :onMouseOver => \"MM_swapImage('edit1','','/images/edit_down.gif',1);\"%>" 
			file.puts "<% end %>\n"
			...
			file.puts "  <% if $allow_editing %>\n"
			#file.puts "    <%= link_to image_tag(\"edit\", :size => \"80x21\", :border => 0), :action => \"edit\", :id => @"+name+".id %>"
			file.puts "<%= link_to image_tag(\"edit_up.gif\", :size => \"87x19\", :border => 0, :alt => \"edit\", :name => \"edit\"), {:action => 'edit_dynamic', :id => @"+name+".id}, :onMouseOut => \"MM_swapImgRestore();\", :onMouseOver => \"MM_swapImage('edit','','/images/edit_down.gif',1);\"%>" 
			file.puts "<% end %>\n"
      ...
      file.puts "</div>\n"
			...
			name_value['addrelated'].each_key do |key|
				...
				file.puts "  <% if @"+name+"."+key.to_s.pluralize+" != nil %>\n"
					file.puts "<% if @"+name+"."+key.to_s.pluralize+".size > 5 %>\n"
					file.puts "<div id=\"scroll"+key.to_s.pluralize+"\" style=\"height:100;background-color:e6e6e6;border:0px solid;overflow:auto;\">\n"
					file.puts "<% end %>\n"
				file.puts "    <% for "+key.to_s.singularize+" in @"+name+"."+key.to_s.pluralize+" %>\n"
					file.puts "  <%= link_to_remote "+key.to_s.singularize+".displayInfo, :update => \""+name+"_"+key.to_s.pluralize+"\", :url=> {:controller=> \""+key.to_s.pluralize+"\", :action => \"edit_dynamic\", :id => "+key.to_s.singularize+".id,:params => {:level => @params['level'], :relatedtype =>'"+key.to_s.singularize+"',:internal => '"+name.pluralize+"', 'pk' => @params['pk'] != nil ? @params['pk'] : @params['id'] } } %><br>\n"
				...
				file.puts "  <% if @"+name+"."+key.to_s+" != nil %>\n"
					file.puts "  <%= link_to_remote @"+name+"."+key.to_s.singularize+".displayInfo, :update => \""+name+"_"+key.to_s.pluralize+"\", :url=> {:controller=> \""+key.to_s.pluralize+"\", :action => \"edit_dynamic\", :id => @"+name+"."+key.to_s.singularize+".id,:params => {:level => @params['level'], :relatedtype =>'"+key.to_s.singularize+"',:internal => '"+name.pluralize+"', 'pk' => @params['pk'] != nil ? @params['pk'] : @params['id'] } } %><br>\n"
				file.puts "  <% end %>\n"
